...
from tomo.objects import Tomogram, SetOfTomograms, SetOfCTFTomoSeries, TiltSeries

from imod import Plugin
from imod.protocols import ProtImodBase
from imod.constants import (TLT_EXT, ODD, EVEN, MRC_EXT,
                            OUTPUT_TOMOGRAMS_NAME, NO_TOMO_PROCESSED_MSG, OUTPUT_TS_FAILED_NAME, XF_EXT, DEFOCUS_EXT)
from imod.utils import generateDefocusIMODFileFromObject

logger = logging.getLogger(__name__)


class ProtImodTomoReconstructionCTF3D(ProtImodBase, ProtStreamingBase):
    """
    Tomogram reconstruction procedure based on the IMOD procedure.

    More info:
        https://bio3d.colorado.edu/imod/doc/man/ctf3dsetup.html
        https://bio3d.colorado.edu/imod/doc/man/tilt.html

    """

    _label = 'tomo reconstruction with ctf3d'
            ...
            for ts in inTsSet.iterItems():
                ...
                if tsId not in self.tsReadList and ts.getSize() > 0:  # Avoid processing empty TS (before the Tis are added)
                    ...
                    cId = self._insertFunctionStep(self.convertInputStep,
                                                   ...
                                                   prerequisites=[],
                                                   needsGPU=False)
                    ctfId = self._insertFunctionStep(self.computeCTF3DStep,
                                                     tsId,
                                                     prerequisites=cId,
                                                     needsGPU=True)
                    recId = self._insertFunctionStep(self.reconstructionStep,
                                                     tsId,
                                                     tomoWidth,
                                                     prerequisites=ctfId,
                                                     needsGPU=True)
    ...
    def computeCTF3DStep(self, tsId):
        logger.info(cyanStr(f'===> tsId = {tsId}: reconstructing the tomogram...'))

        try:
            self.ctfcomFile(tsId)
        except Exception as e:
            self.failedItems.append(tsId)
            logger.error(redStr(f'The creation of the ctf .com file failed for tsId {tsId} -> {e}'))

        try:
            self.setup3dCTFandTiltStep(tsId)
        except Exception as e:
            self.failedItems.append(tsId)
            logger.error(redStr(f'The ctf3dsetup or tilt.com failed for tsId {tsId} -> {e}'))


    def reconstructionStep(self, tsId, tomoWidth):
        logger.info(cyanStr(f'===> tsId = {tsId}: reconstructing the tomogram...'))
        try:
            self.ctfcomFile(tsId)
        except Exception as e:
    ...
    def ctfcomFile(self, tsId):
        """
        Create ctfcorrection.com file for your reconstruction folder.
        """
        ts = self.getCurrentItem(tsId)
        acq = ts.getAcquisition()
        fnTs = ts.getFirstItem().getFileName()
        fnOUt = self._getExtraPath(tsId, f'{tsId}_ctfcorr.mrc')

        tltFilePath = self.getExtraOutFile(tsId, ext=TLT_EXT)
        logger.info(cyanStr(f"\t--> Generating the angles tlt file {tltFilePath} ..."))
        ts.generateTltFile(tltFilePath)

        ctfTomoSeries = self.inputCTF.get()(tsId)
        defocusFile = self.getExtraOutFile(tsId, ext=DEFOCUS_EXT)
        logger.info(cyanStr(f"\t--> Generating defocus file {defocusFile}..."))
        generateDefocusIMODFileFromObject(ctfTomoSeries, defocusFile)

        xfFile = self.getExtraOutFile(tsId, ext=XF_EXT)
        logger.info(cyanStr(f"\t--> Generating xf file {xfFile}..."))
        ts.writeXfFile(xfFile)

        ctfParams = ['#ctfphaseflip',
                   '$setenv IMOD_OUTPUT_FORMAT MRC',
                   '$ctfphaseflip -StandardInput',
                   f'InputStack  {fnTs}',
                   f'AngleFile   {tltFilePath}',
                   f'OutputFileName	{fnOUt}',
                   f'TransformFile   {xfFile}',
                   f'DefocusFile    {defocusFile}',
                   f'Voltage {acq.getVoltage()}',
                   f'SphericalAberration {acq.getSphericalAberration()}',
                   f'DefocusTol	{self.defocusTol.get()}',
                   f'PixelSize {ts.getSamplingRate()/ 10.0}',
                   f'AmplitudeContrast	{acq.getAmplitudeContrast()}',
                   'InterpolationWidth	15',
                   'ActionIfGPUFails	1,2',
                   '$if (-e ./savework) ./savework']

        fnCTFcommFile = self._getExtraPath(tsId, 'ctfcorrection.com"')
        with open(fnCTFcommFile, 'w+') as file:
            file.truncate(0)
            file.write('\n'.join(ctfParams))

        return fnCTFcommFile

    def setup3dCTFandTiltStep(self, tsId):

        ts = self.getCurrentItem(tsId)

        xdim, ydim, n = ts.getDimensions()
        ...
        paramsCTF3dsetup = {
            '-th': self.getTmpOutFile(tsId, ext=MRC_EXT),
            '-pa': self.getExtraOutFile(tsId, ext=MRC_EXT),
            'tilt': ''
        }
        self.runProgram('ctf3dsetup', paramsCTF3dsetup)


    def reconstruction3dCTFStep(self, tsId):
        logger.info(cyanStr(f"\t--> Reconstructing tsId={tsId}..."))
        paramsProcesschunks = {
            'localhost': '',
            'ctf3d': self.getExtraOutFile(tsId+'rec', ext=MRC_EXT)
        }
        self.runProgram('processchunks', paramsProcesschunks)
        logger.info(cyanStr(f"\t--> Reconstruction tsId={tsId} finished"))


        paramsClip =  'rotx'
        paramsClip += self.getExtraOutFile(tsId+'rec', ext=MRC_EXT)
        paramsClip += self.getExtraOutFile(tsId + 'rec_rot', ext=MRC_EXT)

        self.runProgram('clip', paramsClip)
